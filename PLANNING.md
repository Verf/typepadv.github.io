# 形码练习平台（typepadv.github.io）项目规划

一个可通过 GitHub Pages 部署的综合形码练习平台。平台支持多种内置输入方案（宇浩星陈、灵铭 / Gallming）、字根记忆、文章跟打、自定义跟打文本、提示码表和自定义键盘布局（Gallman）。默认入口仍为文章跟打，字根记忆通过 `#/roots` 进入。
...
### 2. 功能范围（MVP）
核心功能：
1. 字根记忆：教学卡→闭卷回忆，支持智能进阶、只练大码、只练完整根码，按今日待复习/新字根/薄弱项/键位筛选；进度按方案和资产版本隔离，采用 2/5/10 题内强化与 1/3/7/14/30 天复习
2. 平台导航：文章跟打与字根记忆使用共享的方案、键盘布局设置；拆分练习、单字编码和学习统计作为后续模块入口
3. 跟打区：显示待打字文本，实时渲染已打/当前字/未打区，当前字高亮，支持回改（退格）、重新开始
4. 实时统计：击键 KPM、键准（无回改口径）、回改次数、错字率、用时、正确率，实时刷新
5. 提示码表：当前字的编码实时提示，内置宇浩星陈 + 灵铭两套方案 + 支持上传自定义码表
6. 键盘布局：QWERTY（默认）+ Gallman 两套内置，目标键高亮 + 按键三态反馈 + 指法分区着色（可开关）
7. 跟打文本管理：内置示例文本 + 自定义文本导入（粘贴/上传 txt），按段/按行拆分
8. 历史记录：每次练习成绩（KPM/键准/正确率/用时/文本）存 localStorage，可查看列表
...
### 5. 提示码表
- 内置**宇浩星陈**全量码表（shurufa.app/mabiao-star.txt）+ **灵铭（gallming）**纯规范声码五码正式版（兼容性遗留文件名 mabiao-ling.txt，灵明×Gallman 重排），格式「编码\t汉字」，编码在左；灵铭资产可用 `npm run sync:gallming` 从 gallming 上游正式交付物重建
