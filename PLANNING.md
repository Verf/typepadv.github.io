...
一个可通过 GitHub Pages 部署的中文跟打练习器，核心目标：练习宇浩星陈输入方案，支持自定义跟打文本、提示码表（宇浩星陈）、自定义键盘布局（Gallman）。
...
3. 提示码表：当前字的宇浩星陈编码实时提示（顶栏/侧栏），内置宇浩星陈全量码表 + 支持上传自定义码表
...
### 5. 提示码表
- 内置**宇浩星陈**全量码表（shurufa.app/mabiao-star.txt），格式「编码\t汉字」，编码在左
